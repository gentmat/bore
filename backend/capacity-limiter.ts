  ...
  try {
    ...
    if (fleetStats.serverCount > 0) {
      ...
      const reservedSlots = Math.floor(
        (fleetStats.totalCapacity * CAPACITY_CONFIG.reservedCapacityPercent) /
          100,
      );

      return {
        hasCapacity:
          fleetStats.totalLoad < fleetStats.totalCapacity - reservedSlots,
    ...
    const activeTunnels = instances.rows[0]
      ? parseInt(instances.rows[0].active)
      : 0;
    const totalCapacity = CAPACITY_CONFIG.totalSystemCapacity;
    const availableCapacity = totalCapacity - activeTunnels;
    const reservedSlots = Math.floor(
      (totalCapacity * CAPACITY_CONFIG.reservedCapacityPercent) / 100,
    );
    ...
    return {
      ...
      reason:
        activeTunnels >= maxTunnels
          ? `Plan limit reached (${maxTunnels} tunnels)`
          : null,
    };
  } catch (error) {
 ...
 */
export async function getLeastLoadedServer(
  availableServers: ServerInfo[],
): Promise<ServerInfo | null> {
  logger.warn(
    "getLeastLoadedServer is deprecated, use server-registry.getBestServer()",
  );
  ...
  try {
    ...
    if (!systemCheck.hasCapacity) {
      res.status(503).json({
        ...
        message:
          "System at capacity. Please try again later or upgrade your plan.",
        details: {
...
function generateCapacityAlerts(
  capacityInfo: SystemCapacityCheck,
): CapacityAlert[] {
...
export function updateCapacity(newTotalCapacity: number): void {
  logger.warn(
    "updateCapacity() modifies runtime config only. Update TOTAL_SYSTEM_CAPACITY env var for persistent changes.",
  );
  (CAPACITY_CONFIG as { totalSystemCapacity: number }).totalSystemCapacity =
    newTotalCapacity;
  logger.info(
    `✅ System capacity updated to ${newTotalCapacity} tunnels (runtime only)`,
  );
}
