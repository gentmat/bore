    ...
    for (const srv of servers) {
      const result = await client.query(
        ...
        [
          ...
          srv.port,
          srv.location,
          srv.max_bandwidth_mbps,
          srv.max_concurrent_tunnels,
          srv.status,
        ],
      );
...
async function clearSeedData(pool: Pool): Promise<void> {
  ...
  try {
    await client.query("BEGIN");
    await client.query(
      "TRUNCATE refresh_tokens, tunnel_tokens, health_metrics, status_history, alert_history, instances, bore_servers, waitlist, users CASCADE",
    );
    await client.query("COMMIT");
    // eslint-disable-next-line no-console
