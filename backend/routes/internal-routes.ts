...
router.post('/validate-key', requireInternalApiKey, validate(schemas.validateKey), async (req: Request, res: Response): Promise<void> => {
...
router.post('/instances/:id/tunnel-connected', requireInternalApiKey, validate(schemas.tunnelConnected), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (remotePort !== undefined && remotePort !== null) {
      const effectiveHost = instance.server_host || instance.serverHost || BORE_SERVER_HOST;
    ...
    ErrorResponses.internalError(res, 'Failed to update tunnel status', req.id);
  }
});
...
router.post('/instances/:id/tunnel-disconnected', requireInternalApiKey, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (currentToken) {
      await db.deleteTunnelToken(currentToken);
      await db.updateInstance(instance.id, {
