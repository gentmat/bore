...
function updateInstanceCards(newInstances: Instance[], container: HTMLElement, emptyState: HTMLElement): void {
...
function createInstanceCard(instance: Instance): HTMLElement {
...
function updateInstanceCard(card: HTMLElement, instance: Instance): void {
    const nameEl = card.querySelector('[data-field="name"]') as HTMLElement;
    if (nameEl) {
    ...
    const actionsContainer = card.querySelector('.instance-actions') as HTMLElement;
    if (actionsContainer) {
...
function bindCardActions(card: HTMLElement, instance: Instance): void {
    ...
    const connectButton = card.querySelector('[data-action="connect"]') as HTMLButtonElement;
    if (connectButton && !connectButton.disabled) {
        connectButton.addEventListener('click', () => connectInstance(instance.id));
    }
}

function getActionsTemplate(instance: Instance): string {
...
function viewTunnel(instanceId: string, instanceName: string, localPort: number): void {
...
(window as any).viewTunnel = viewTunnel;
