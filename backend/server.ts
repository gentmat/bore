...
import { metricsMiddleware, generatePrometheusMetrics, updateInstanceStatusCounts } from './metrics';
import { initializeWebSocket, broadcastStatusChange as wsBroadcast } from './websocket';
...
app.use('/api/instances*', (req: Request, res: Response, next: NextFunction) => {
  if (!req.path.startsWith('/api/v1/')) {
    const newPath = req.path.replace('/api/', '/api/v1/');
  ...
  try {
    ...
    if (config.redis.enabled) {
      try {
        ...
        logger.warn('⚠️  Redis initialization failed - falling back to in-memory state', error as Error);
